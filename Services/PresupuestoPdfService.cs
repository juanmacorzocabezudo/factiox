...
{
    byte[] GenerarPresupuestoPdf(Presupuesto presupuesto, Cliente cliente, ConfiguracionEmpresa? config, bool paraEmail = true);
}
...
public class PresupuestoPdfService : IPresupuestoPdfService
{
    ...
    public byte[] GenerarPresupuestoPdf(Presupuesto presupuesto, Cliente cliente, ConfiguracionEmpresa? config, bool paraEmail = true)
    {
        ...
        var documento = Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Header().Column(column =>
                {
                    column.Item().Row(row =>
                    {
                        ...
                        row.RelativeItem().Column(col =>
                        {
                            ...
                            col.Item().Background("#5cb85c").Padding(2).Column(c =>
                            {
                                c.Item().AlignCenter().Text("PRESUPUESTO").FontSize(fuenteGrande).Bold().FontColor(Colors.White);
                                c.Item().AlignCenter().Text(presupuesto.Numero).FontSize(fuenteBase).FontColor(Colors.White);
                            });
                            ...
                            {
                                c.Item().Text("CLIENTE").Bold().FontSize(fuenteBase).FontColor("#5cb85c");
                    ...
                    {
                        row.AutoItem().Text($"Fecha: {presupuesto.Fecha:dd/MM/yyyy}").FontSize(fuentePequena);
                        if (presupuesto.FechaValidez.HasValue)
                        {
                            row.AutoItem().PaddingLeft(5).Text($"Válido hasta: {presupuesto.FechaValidez.Value:dd/MM/yyyy}").FontSize(fuentePequena);
                        }
                        row.AutoItem().PaddingLeft(5).Text($"Estado: {presupuesto.Estado}").FontSize(fuentePequena);
                    });
                ...
                page.Content().Column(column =>
                {
                    ...
                    // Notas si existen
                    if (!string.IsNullOrEmpty(presupuesto.Notas))
                    {
                        column.Item().Background("#fffbf0").BorderLeft(2).BorderColor("#5cb85c")
                            .Padding(paraEmail ? 10 : 5).Text($"Notas: {presupuesto.Notas}").FontSize(fuenteBase).Italic();
                        column.Item().PaddingVertical(paraEmail ? 8 : 8);
                    }

                    // Tabla de líneas
                    if (presupuesto.Lineas != null && presupuesto.Lineas.Any())
                    {
                        column.Item().Table(table =>
                        {
                            ...
                            table.Header(header =>
                            {
                                ...
                                header.Cell().Background("#5cb85c").Padding(headerPadding)
                    ...
                    // Totales
                    column.Item().AlignRight().Width(paraEmail ? 250 : 280).Column(col =>
                    {
                        col.Item().Table(table =>
                        {
                            ...
                            table.Cell().Background("#5cb85c").Padding(totalPadding).Text("TOTAL:").Bold().FontSize(fuenteMedia).FontColor(Colors.White);
                            table.Cell().Background("#5cb85c").Padding(totalPadding).AlignRight()
                                .Text(presupuesto.Total.ToString("C2")).Bold().FontSize(fuenteMedia).FontColor(Colors.White);
                        });
                        
                        // Mensaje IVA no incluido
                        col.Item().PaddingTop(paraEmail ? 5 : 3).AlignRight()
                            .Text("* IVA no incluido").FontSize(fuentePequena).Italic().FontColor(Colors.Grey.Darken1);
                    });

                    // Condiciones de pago si existen
                    if (!string.IsNullOrEmpty(presupuesto.CondicionesPago))
                    {
                        column.Item().PaddingTop(paraEmail ? 2 : 8);
                        column.Item().Background(Colors.Grey.Lighten4).Padding(paraEmail ? 2 : 5).Column(col =>
                        {
                            col.Item().Text("Condiciones de Pago:").Bold().FontSize(fuenteBase).FontColor("#5cb85c");
                            col.Item().Text(presupuesto.CondicionesPago).FontSize(fuentePequena);
                        });
                    }
                    ...
                    column.Item().PaddingTop(paraEmail ? 2 : 10).BorderTop(1).BorderColor(Colors.Grey.Medium)
                        .PaddingTop(paraEmail ? 1 : 5).AlignCenter().Column(col =>
                        {
                            ...
                            col.Item().Text("Este presupuesto es válido durante el período indicado")
                ...
                page.Footer().Column(footerColumn =>
                {
                    ...
                    footerColumn.Item().BorderTop(1).BorderColor(Colors.Grey.Lighten1).PaddingTop(1)
                        .AlignCenter().Column(legalCol =>
                        {
                            ...
                            {
                                txt.Span($"Los datos personales facilitados serán tratados por {config?.NombreEmpresa ?? "FactioX"} con la finalidad de gestionar la relación comercial. ")
                            ...
                            legalCol.Item().PaddingTop(0.5f).Text(txt =>
                            {
                                ...
                                txt.Span("Este presupuesto no tiene efectos fiscales hasta su conversión en factura.")
                                    .FontSize(fuenteMuyPequena - 1).FontColor(Colors.Grey.Darken1);
                            });
                        });
